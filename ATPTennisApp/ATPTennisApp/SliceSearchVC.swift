...
class SliceSearchVC: CustomNavibar, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    var nameList: [String] = []
    var nameSearchingList: [String] = []
    
    var isSearching: Bool = false
    var slService: SliceService?
    ...
    func getPlayerNameFromServer(){
        if slService == nil {
            slService = SliceService()
        }
        Utils.showProgress(self.tableView)
        slService?.requestSliceFromServer({ (result, object) in
            self.nameList = ((self.slService?.listName)?.sorted())!
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let sliceDetailVC = SliceDetailVC(nibName: "SliceDetailVC", bundle: nil)
        if isSearching == true {
            for object in (slService?.listSliceService)! {
                let playerName = nameSearchingList[indexPath.row]
                let ob = object as! SliceModel
                if ob.name == playerName {
                    sliceDetailVC.objectModel = ob
                    break
                }
            }
        ...
        self.navigationController?.pushViewController(sliceDetailVC, animated: true)
    }
