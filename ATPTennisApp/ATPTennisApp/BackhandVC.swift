...
class BackhandVC: CustomNavibar, UITableViewDelegate, UITableViewDataSource {

    @IBOutlet weak var tableView: UITableView!
    
    var backhandService: BackhandService?
    ...
    func getBackhandFromServer(){
        if backhandService == nil{
            self.backhandService = BackhandService()
            
        }
        Utils.showProgress(self.view)
        backhandService?.requestBackhandFromServer({ (result, object) in
            Utils.hideProgress(self.view)
            for item in (self.backhandService?.listName.sorted())!{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        var x = 0
        for object in (backhandService?.listBackhandService)!{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let objectId = listRow.object(at: indexPath.row) as! BackhandModel
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let backhandDetail = BackhandDetailVC(nibName: "BackhandDetailVC", bundle: nil)
        ...
        backhandDetail.objectModel = listRow.object(at: indexPath.row) as? BackhandModel
        
        self.navigationController?.pushViewController(backhandDetail, animated: true)
    }
    ...
    func searchPlayer() -> Void {
        self.pressSearch = {(sender: AnyObject) in
            ...
            let alertName = UIAlertAction(title: "Search by Name", style: .default, handler: { (action: UIAlertAction) in
                ...
                let searchPlayer = BackhandSearchVC(nibName: "BackhandSearchVC", bundle: nil)
